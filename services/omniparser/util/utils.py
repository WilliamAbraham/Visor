...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    ...
    if use_paddleocr:
        ...
        # For a single image, result is a list containing one list of detections
        # So we need result[0] to get the actual detections
        ...
        if result is None:
            ...
            # result[0] contains the list of detections for the first image
            detections = result[0]
            
            # Parse each detection: format is [[bbox_coords], (text, confidence)]
            for item in detections:
                if item is None or len(item) < 2:
                    continue
                try:
                    # Standard format: [bbox_coords, (text, confidence)]
                    bbox = item[0]  # This is [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]
                    text_info = item[1]  # This is (text, confidence)
                    
                    # Handle both tuple/list and other formats for text_info
                    if isinstance(text_info, (list, tuple)) and len(text_info) >= 2:
                        ...
                        # Fallback: assume text_info is just text, use default confidence
                        confidence = 1.0
